...
def get_pymupdf_text(pdf_path):
    ...
    """
    if pymupdf is None:
        raise ImportError("PyMuPDF is required for PyMuPDF text extraction.")
...
def process_pdf(path_pdf, threshold_num_repetitions=5, output_dir=None, text_source="both", **params):
    """
    ...
    output_dir (str): Optional directory where outputs should be written.
    text_source (str): "nougat", "pymupdf", or "both".
    **params: Additional parameters to pass to the nougat extraction method.
    ...
    output_dir = Path(output_dir) if output_dir else p.parent
    output_dir.mkdir(parents=True, exist_ok=True)

    raw_output_nougat_file = output_dir / (p.stem + ".raw.nougat.txt")
    raw_pymupdf_file = output_dir / (p.stem + ".raw.pymupdf.txt")
    processed_output_nougat_file = output_dir / (
        p.stem + ".processed.nougat.txt"
    )
    processed_output_pymupdf_file = output_dir / (
        p.stem + ".processed.pymupdf.txt"
    )
    doi_file = output_dir / (p.stem + ".doi.txt")
    text_source = text_source.lower().strip()
    if text_source not in {"nougat", "pymupdf", "both"}:
        raise ValueError("text_source must be 'nougat', 'pymupdf', or 'both'.")

    ## Nougat
    if text_source in {"nougat", "both"}:
        err = nougat_process_pdf(p, out=output_dir, **params)
        print(err)
        rename_file(output_dir / (p.stem + ".mmd"), raw_output_nougat_file)
    ...
    ## PyMuPDF
    if text_source in {"pymupdf", "both"}:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        help="Directory where extracted text files should be written.",
    )
    parser.add_argument(
        "--text-source",
        choices=["nougat", "pymupdf", "both"],
        default="both",
        help="Text extraction backend to run.",
    )
